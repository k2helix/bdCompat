...
class BdApi {
    ...
    static __getPluginConfig(pluginName) {
        ...
        if (typeof BdApi.__pluginData[pluginName] === "undefined")
            if (!fs.existsSync(configPath)) {
                ...
                try {
                    BdApi.__pluginData[pluginName] = JSON.parse(
                        fs.readFileSync(configPath)
                    );
                } catch (e) {
                    BdApi.__pluginData[pluginName] = {};
                    BdApi.__warn(
                        `${pluginName} has corrupted or empty config file, loaded as {}`
                    );
                }
            }
    ...
    static __savePluginConfig(pluginName) {
        ...
        fs.writeFileSync(
            configPath,
            JSON.stringify(BdApi.__pluginData[pluginName], null, 2)
        );
    }
    ...
    static async alert(title, children) {
        return BdApi.showConfirmationModal(title, children, {
            cancelText: null,
        });
    }
    ...
    static async showConfirmationModal(title, children, options = {}) {
        ...
        const Markdown = await getModule(
            (m) => m.displayName === "Markdown" && m.rules
        );
    ...
    static showToast(content, options = {}) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                toastElem.remove();
                if (!document.querySelectorAll(".bd-toasts .bd-toast").length)
                    toastWrapper.remove();
            }, 300);
        }, timeout);
    }
    ...
    static __createToastWrapper() {
        ...
        if (!toastWrapperElem) {
            ...
            const boundingElement = document.querySelector(
                Object.keys(DiscordElements)
                    .map((component) => DiscordElements[component])
                    .join(", ")
            );
            ...
            toastWrapper.style.setProperty(
                "width",
                boundingElement ? boundingElement.offsetWidth + "px" : "100%"
            );
            toastWrapper.style.setProperty(
                "left",
                boundingElement
                    ? boundingElement.getBoundingClientRect().left + "px"
                    : "0px"
            );
            toastWrapper.style.setProperty(
                "bottom",
                (document.querySelector(DiscordElements.chat)
                    ? document.querySelector(DiscordElements.chat)
                          .offsetHeight + 20
                    : 80) + "px"
            );
    ...
    static findModuleByPrototypes(...protos) {
        return getModule(
            (module) =>
                module.prototype &&
                protos.every(
                    (proto) => typeof module.prototype[proto] !== "undefined"
                ),
            false
        );
    }
        ...
        if (options.instead) {
            ...
            what[methodName] = function () {
                ...
                const tempRet = BdApi.suppressErrors(
                    options.instead,
                    "`instead` callback of " + what[methodName].displayName
                )(data);
    ...
    static __injectBefore(data, origMethod) {
        const patchID = `bd-patch-before-${data.displayName.toLowerCase()}-${crypto
            .randomBytes(4)
            .toString("hex")}`;
        ...
        inject(
            ...
            function beforePatch(args, res) {
                const patchData = {
                    ...
                    originalMethod: origMethod,
                    callOriginalMethod: () =>
                        (patchData.returnValue = patchData.originalMethod.apply(
    ...
    static __injectAfter(data, origMethod) {
        const patchID = `bd-patch-after-${data.displayName.toLowerCase()}-${crypto
            .randomBytes(4)
            .toString("hex")}`;
    ...
    static get settings() {
        // mess
        return {
            "Custom css live update": { id: "bda-css-0" },
            "Custom css auto udpate": { id: "bda-css-1" },
            "BetterDiscord Blue": { id: "bda-gs-b" },
            "Public Servers": { id: "bda-gs-1" },
            "Minimal Mode": { id: "bda-gs-2" },
            "Voice Mode": { id: "bda-gs-4" },
            "Hide Channels": { id: "bda-gs-3" },
            "Dark Mode": { id: "bda-gs-5" },
            "Voice Disconnect": { id: "bda-dc-0" },
            "24 Hour Timestamps": { id: "bda-gs-6" },
            "Colored Text": { id: "bda-gs-7" },
            "Normalize Classes": { id: "fork-ps-4" },
            "Content Error Modal": { id: "fork-ps-1" },
            "Show Toasts": { id: "fork-ps-2" },
            "Scroll To Settings": { id: "fork-ps-3" },
            "Automatic Loading": { id: "fork-ps-5" },
            "Developer Mode": { id: "bda-gs-8" },
            "Copy Selector": { id: "fork-dm-1" },
            "React DevTools": { id: "reactDevTools" },
            "Enable Transparency": { id: "fork-wp-1" },
            "Window Frame": { id: "fork-wp-2" },
            "Download Emotes": { id: "fork-es-3" },
            "Twitch Emotes": { id: "bda-es-7" },
            "FrankerFaceZ Emotes": { id: "bda-es-1" },
            "BetterTTV Emotes": { id: "bda-es-2" },
            "Emote Menu": { id: "bda-es-0" },
            "Emoji Menu": { id: "bda-es-9" },
            "Emote Auto Capitalization": { id: "bda-es-4" },
            "Show Names": { id: "bda-es-6" },
            "Show emote modifiers": { id: "bda-es-8" },
            "Animate On Hover": { id: "fork-es-2" },
        };
    }
