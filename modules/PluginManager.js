...
module.exports = class BDPluginManager {
    constructor(pluginsFolder, settings) {
        ...
        FluxDispatcher.subscribe(
            "CHANNEL_SELECT",
            (this.channelSwitch = () => this.fireEvent("onSwitch"))
        );
    ...
    destroy() {
        ...
        if (this.channelSwitch)
            FluxDispatcher.unsubscribe("CHANNEL_SELECT", this.channelSwitch);
    ...
    isEnabled(pluginName) {
        ...
        if (!plugin)
            return this.__error(
                null,
                `Tried to access a missing plugin: ${pluginName}`
            );
    ...
    startPlugin(pluginName) {
        ...
        if (!plugin)
            return this.__error(
                null,
                `Tried to start a missing plugin: ${pluginName}`
            );
    ...
    stopPlugin(pluginName) {
        ...
        if (!plugin)
            return this.__error(
                null,
                `Tried to stop a missing plugin: ${pluginName}`
            );
    ...
    reloadPlugin(pluginName) {
        ...
        if (!plugin)
            return this.__error(
                null,
                `Tried to reload a missing plugin: ${pluginName}`
            );
    ...
    enablePlugin(pluginName) {
        ...
        if (!plugin)
            return this.__error(
                null,
                `Tried to enable a missing plugin: ${pluginName}`
            );
    ...
    disablePlugin(pluginName) {
        ...
        if (!plugin)
            return this.__error(
                null,
                `Tried to disable a missing plugin: ${pluginName}`
            );
    ...
    loadPlugin(pluginName) {
        ...
        if (!existsSync(pluginPath))
            return this.__error(
                null,
                `Tried to load a nonexistant plugin: ${pluginName}`
            );
        ...
        try {
            ...
            try {
                ...
                if (window.bdplugins[plugin.getName()])
                    window.bdplugins[plugin.getName()].plugin.stop();
                delete window.bdplugins[plugin.getName()];
                window.bdplugins[plugin.getName()] = {
                    plugin,
                    __filePath: pluginPath,
                    ...meta,
                };
                ...
                if (plugin.load && typeof plugin.load === "function")
                    try {
                        ...
                        if (pluginName === "0PluginLibrary")
                            this.__patchZLibPatcher();
                    } catch (err) {
                        this.__error(
                            err,
                            `Failed to preload ${plugin.getName()}`
                        );
                    }

                this.__log(
                    `Loaded ${plugin.getName()} v${plugin.getVersion()} by ${plugin.getAuthor()}`
                );
            } catch (e) {
    ...
    delete(pluginName) {
        ...
        if (!plugin)
            return this.__error(
                null,
                `Tried to delete a missing plugin: ${pluginName}`
            );
    ...
    fireEvent(event, ...args) {
        for (const plug in window.bdplugins) {
            ...
            try {
                ...
                this.__error(
                    err,
                    `Could not fire ${event} event for ${plugin.name}`
                );
            }
        }
    }
    ...
    addMissingGetMethods(pluginName, meta, plugin) {
        ...
        if (!plugin.getDescription)
            plugin.getDescription = () => meta.description || "";
        if (!plugin.getVersion)
            plugin.getVersion = () => meta.version || "0.0.0";
        if (!plugin.getAuthor)
    ...
    __patchZLibPatcher() {
        ...
        if (!window?.ZLibrary?.Patcher)
            return this.__error(null, "Failed to patch ZLibrary Patcher");
        ...
        inject(
            ...
            (args) => {
                ...
                if (
                    orig &&
                    !(orig instanceof origFunction) &&
                    orig instanceof _window.Function
                )
                    window.Function = _window.Function;
                return args;
            },
            true
        );
